...
const SuperAdminDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_SERVER_DOMAIN}/superadmin/stats`,
        {
  ...
  const fetchUsers = async (search = "") => {
    try {
      ...
      const { data } = await axios.get(
        ...
        }/superadmin/users?search=${search}`,
        {
          withCredentials: true,
        }
      );
  ...
  const fetchBlogs = async (search = "") => {
    try {
      ...
      const { data } = await axios.get(
        `${
          import.meta.env.VITE_SERVER_DOMAIN
        }/superadmin/blogs?search=${search}`,
        {
          withCredentials: true,
        }
      );
  ...
  const fetchAdmins = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `${import.meta.env.VITE_SERVER_DOMAIN}/superadmin/admins`,
        {
          withCredentials: true,
        }
      );
  ...
  const updateUserRole = async (userId, role) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_SERVER_DOMAIN}/superadmin/users/${userId}/role`,
        { role }
      );
  ...
  const deleteUser = async (userId) => {
    if (
      ...
      try {
        await axios.delete(
          `${import.meta.env.VITE_SERVER_DOMAIN}/superadmin/users/${userId}`
        );
        fetchUsers(userSearch);
      } catch (error) {
  ...
  const deleteBlog = async (blogId) => {
    if (
      ...
      try {
        await axios.delete(
          `${import.meta.env.VITE_SERVER_DOMAIN}/superadmin/blogs/${blogId}`
        );
  ...
  const testPing = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_SERVER_DOMAIN}/ping`,
        {
          timeout: 5000,
  ...
  const fetchDebugInfo = async () => {
    try {
      ...
      const requestPromise = axios.get(
        `${import.meta.env.VITE_SERVER_DOMAIN}/debug/auth`,
        {
          withCredentials: true,
      ...
      if (error.code === "NETWORK_ERROR" || !error.response) {
        ...
        errorInfo.mobileIssue =
          "Request blocked - possible CORS or cookie issue";
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex gap-4">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && fetchBlogs(blogSearch)
                      }
                      className="input-box"
                    />
                    <button
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h3 className="font-medium">
                                {admin.personal_info.fullname}
                              </h3>
                              <p className="text-dark-grey">
                                @{admin.personal_info.username}
                              </p>
                              <p className="text-sm text-dark-grey">
                                {admin.personal_info.email}
                              </p>
                            </div>
                          </div>
            ...
            <div>
              ...
              <div className="bg-white border border-grey rounded-lg p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex gap-2">
                    <button onClick={testPing} className="btn-light">
                      Test Connection
                    </button>
                    <button
                  ...
                  >
                    <h4
                      className={`font-medium ${
                        pingTest.success ? "text-green" : "text-red"
                      }`}
                    >
                      Server Connection:{" "}
                      {pingTest.success ? "SUCCESS" : "FAILED"}
                    </h4>
                    {pingTest.success ? (
                ...
                {debugLoading ? (
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      <h3
                        className={`font-medium ${
                          debugInfo.authenticated ? "text-green" : "text-red"
                        }`}
                      >
                        Authentication Status:{" "}
                        {debugInfo.authenticated ? "SUCCESS" : "FAILED"}
                      </h3>
                    </div>

                    {debugInfo.error && (
                      <div className="p-4 bg-red/10 border border-red/20 rounded-lg">
                        <h4 className="font-medium text-red mb-2">
                          Error Details:
                        </h4>
                        ...
                        {debugInfo.requestDetails && (
                          <div className="mt-3 text-xs space-y-1">
                            <p>
                              <strong>URL:</strong>{" "}
                              {debugInfo.requestDetails.url}
                            </p>
                            <p>
                              <strong>Network Error:</strong>{" "}
                              {debugInfo.requestDetails.networkError
                                ? "Yes"
                                : "No"}
                            </p>
                            <p>
                              <strong>Timeout:</strong>{" "}
                              {debugInfo.requestDetails.timeout ? "Yes" : "No"}
                            </p>
                        ...
                        <div className="p-4 bg-grey/10 border border-grey rounded-lg">
                          ...
                          <div className="text-sm space-y-1">
                            <p>
                              ...
                              {debugInfo.headers?.userAgent || "Not available"}
                            </p>
                            <p>
                              <strong>Origin:</strong>{" "}
                              {debugInfo.headers?.origin || "Not available"}
                            </p>
                            <p>
                              <strong>Referer:</strong>{" "}
                              {debugInfo.headers?.referer || "Not available"}
                            </p>
                          </div>
                        </div>
                      </>
                    ...
                    <div className="text-xs text-dark-grey mt-4">
                      <p>
                        💡 <strong>Tip:</strong> If authentication fails on
                        mobile, check if cookies are being sent properly.
                      </p>
                      <p>
                        💡 <strong>Mobile Issue:</strong> If you can't publish
                        blogs, this will show authentication status.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-8 text-dark-grey">
                    <p>
                      Click "Test Auth" to check authentication status and debug
                      mobile issues
                    </p>
                  </div>
                )}
